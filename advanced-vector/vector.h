...
class Vector {
   public:
    ...
    iterator Emplace(const_iterator pos, Args&&... args) {
        ...
        if (size_ == Capacity()) {
            ...
            try {
                InitializeMemory(data_.GetAddress(), offset, new_data.GetAddress());
            } catch (...) {
            ...
            try {
                InitializeMemory(data_.GetAddress() + offset, size_ - offset, new_data.GetAddress() + (offset + 1));
            } catch (...) {
                std::destroy_n(new_data.GetAddress(), offset + 1);
                throw;
            }
    ...
    void Reserve(size_t new_capacity) {
        ...
        InitializeMemory(data_.GetAddress(), size_, new_data.GetAddress());
        std::destroy_n(data_.GetAddress(), size_);
   ...
   private:
    ...
    void InitializeMemory(T* data, size_t count, T* new_data) {
        if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>) {
            std::uninitialized_move_n(data, count, new_data);
        } else {
